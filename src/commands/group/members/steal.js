...
const handler = async (sock, msg, from, args, msgInfoObj) => {
    ...
    if (type === "extendedTextMessage" && content.includes("stickerMessage")) {
        let packName = authorName = "WA Utility Bot";
        ...
        try {
            const webpWithMetadata = await WSF.setMetadata(
                packOrAuthor ? packName : evv ? evv : memberData.customStealText || "WA Utility Bot",
                packOrAuthor ? authorName : evv ? "" : memberData.customStealText ? "" : "WA Utility Bot",
                media
            );
            ...
            const buffer = await new Sticker(media)
                .setPack(packOrAuthor ? packName : evv ? evv : memberData.customStealText || "WA Utility Bot")
                .setAuthor(packOrAuthor ? authorName : evv ? "" : memberData.customStealText ? "" : "WA Utility Bot")
        ...
        return sendMessageWTyping(from, { text: `❌ *Reply to a Sticker*` }, { quoted: msg });
    }
}
